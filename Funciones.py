...
LISTA_PALABRAS_POSIBLES = []  # Inicializamos esta lista vacía para luego cargar las palabras
...
# Jugar el juego
def play_game():
    global intentos, secret_word, cuadros_letras, teclas_botones, intento_bloqueado, LISTA_PALABRAS_POSIBLES
...
def borrar_letra():
    for i in range(4, -1, -1):
        if cuadros_letras[intentos][i].get():  # Si hay una letra en el cuadro actual
            cuadros_letras[intentos][i].delete(0, tk.END)  # Borra la letra
            cuadros_letras[intentos][i].focus()  # Enfoca este cuadro
            break
...
def crear_ventana():
    ...
    for intento in range(6):
        ...
        for letra in range(5):
            cuadro = tk.Entry(frame_central, font=("Arial", 16), width=3, justify="center", bg="black", fg="white", highlightbackground="gray", highlightthickness=2)
            cuadro.grid(row=intento+1, column=letra, padx=5, pady=5)
            cuadro.bind("<Key>", lambda event, letra=letra, intento=intento: on_key_press(event, letra, intento))  # Manejar la entrada del teclado
    ...
    for fila, letras in enumerate([letras_fila1, letras_fila2, letras_fila3]):
        for columna, letra in enumerate(letras):
            boton = tk.Button(frame_teclado, text=letra, font=("Arial", 14), width=4, height=2, bg="gray", fg="white", command=lambda l=letra: ingresar_letra(l))
...
# Boton de salida del programa
    ...
    boton_jugar = tk.Button(ventana, text="Jugar de nuevo", font=("Arial", 14), width=10, height=2, bg="green", fg="white", command=resetear_juego)
...
def ingresar_letra(letra):
    ...
    next_widget = widget_actual.tk_focusNext()  # Enfoca el siguiente cuadro
    if next_widget:
        next_widget.focus()