...
from Funciones import crear_ventana, cargar_palabras
import datetime
...
if hora_actual.hour >=20 or hora_actual.hour < 6:
    ...
    instrucciones_ventana.config(bg="white") 
...
def mostrar_instrucciones():
    ...
    if hora_actual.hour >= 20 or hora_actual.hour < 6:
        titulo = tk.Label(
            frame_central, text="LA PALABRA DEL DÍA", font=("Arial", 24),
    ...
    if hora_actual.hour >= 20 or hora_actual.hour < 6:
        instrucciones_label = tk.Label(
            frame_central, text=instrucciones_texto,
    ...
    for i, (palabra, colores) in enumerate(ejemplos):
        for j, (letra, color) in enumerate(zip(palabra, colores)):
            cuadro = tk.Entry(
                frame_central, width=3, font=("Arial", 40), justify="center",
    ...
    empezar_juego_btn = tk.Button(
        frame_central, text="Empezar Juego",
        command=lambda: [instrucciones_ventana.destroy(), iniciar_juego()],
